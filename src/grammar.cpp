...
Grammar::set_map<ProductionSymbol> Grammar::generate_first_sets()
{
...
std::set<ProductionSymbol> Grammar::first(const Production &p)
{
...
std::set<ProductionSymbol> Grammar::first(const ProductionSymbol &p)
{
    ...
    if (auto rule = get_rule(p); rule.has_value()) {
